...
const ClienteModal = ({show, onHide, title, empl, modalType, refreshData}:ClienteModalProps) => {

    //CREATE-UPDATE función handleSaveUpdate 
    const handleSaveUpdate = async (empl: Cliente) => {
    try {
        const isNew = empl.id === 0;
        if (isNew) {
            await ClienteService.createCliente(empl);
        } else {
            await ClienteService.updateCliente(empl.id, empl);
        }
...
const handleDelete = async () => {
    try {
        await ClienteService.deleteCliente(empl.id);
    ...
    const formik = useFormik({
        ...
        return(
            ...
            {modalType === ModalType.DELETE ? (
                ...
                <Modal.Body>
                    <p> ¿Está seguro que desea eliminar al cliente 
