...
import './TablaUnidadMedida.css';


const TablaUnidadMedidas = () => {
    //Variable que va a contener los datos recibidos por la API
    const [unidadMedidas, setUnidadMedidas] = useState<UnidadMedida[]>([]);
    const [unidadMedidaLocal, setUnidadMedidasLocal] = useState<UnidadMedida[]>([]);
    ...
    useEffect(() => {
        //Llamamos a la función para obtener todos las Unidades de Medida declaradas en el service
        const fetchUnidadMedidas = async () => {
            const unidadMedidas = await UnidadMedidaService.getUnidadMedidas();
            const contenido = unidadMedidas.content;
            setUnidadMedidasLocal(contenido);
            setUnidadMedidas(contenido);
            setIsLoading(false);
        };
        fetchUnidadMedidas();
    }, [refreshData]);
    ...
    //Unidad de Medida seleccionada que se va a pasar como prop al Modal
    ...
    //Logica de Modal
    const handleClick = (newTitle: string, unimed: UnidadMedida, modal: ModalType) => {
        setTitle(newTitle);
        setModalType(modal)
        setUnidadMedida(unimed);
        setShowModal(true);
    };
    ...
    useEffect(() => {
        const resultadoFiltrado = unidadMedidas.filter(
            (unimed) => unimed.denominacion.toLocaleLowerCase().includes(textoEscrito.toLocaleLowerCase()) ||
                unimed.abreviatura.toLocaleLowerCase().includes(textoEscrito.toLocaleLowerCase())
        )
        setUnidadMedidasLocal(resultadoFiltrado)
    }, [textoEscrito]);
    ...
    return (
        <>
            <div className="BotonesUnidadMedidas">
                <div id="titulo">
                    <h1>Unidades de Medidas Registradas</h1>
                </div>
                <div id="botones">
            ...
            <Button onClick={() => handleClick("Nuevo Producto",
                ...
                {isLoading ? <Loader /> : (
                    ...
                    <Table id="tablaUnidadMedida">
                        ...
                        <tbody>
                            {unidadMedidaLocal.map(unidadMedida => (
                                <tr key={unidadMedida.id}>

                                    <td> {unidadMedida.denominacion} </td>
                                    <td> {unidadMedida.abreviatura} </td>   
                                    <td> <EditButton onClick={() => handleClick("Editar Unidad de Medida", unidadMedida, ModalType.UPDATE)} /> </td>
                                    <td> <DeleteButton onClick={() => handleClick("Borrar Unidad de Medida", unidadMedida, ModalType.DELETE)} /> </td>
...
export default TablaUnidadMedidas;